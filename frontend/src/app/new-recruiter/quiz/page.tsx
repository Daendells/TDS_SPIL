...
export default function QuizPage() {
  ...
  const [sectionStartTimes, setSectionStartTimes] = useState<Record<number, string>>({});
  ...
  const STORAGE_KEY = `newRecruiterQuizFormData_${assessmentTypeId}`;
  const EXPIRY_MINUTES = 3 * 24 * 60;

  // API call to check seafarer assignment
  const assignmentQuery = useCheckSeafarerAssignment(queryToken, assessmentTypeId);
  ...
  useEffect(() => {
    if (assignmentQuery.data) {
      ...
      setAutoFilledData({
        ...
        rank: data.personalData.rank || "",
        vessel: data.personalData.academyName || "",
      });
      setSeamanCode(data.personalData.seafarerCode || "");
    ...
    try {
      ...
      if (item) {
        ...
        if (parsed.expiresAt && Date.now() <= parsed.expiresAt) {
          ...
          if (data && (data.email || data.token || data.currentStep > 0)) {
            ...
            setTutorialDismissed(data.tutorialDismissed || {});
            setSectionStartTimes(data.sectionStartTimes || {});
          }
        } else {
  ...
  useEffect(() => {
    if (!isClient || currentStep >= totalQuizSteps) {
      return;
    }

    if (email || token || currentStep > 0) {
      try {
        const storageData = {
          value: {
            currentStep,
  ...
  // Initialize section timer only for the active session
  ...
  useEffect(() => {
    ...
    const timerLimitMinutes = currentAssessment.timerLimitMinutes ?? 0;

    const sectionStartTime = sectionStartTimes[quizStepIndex] ?? new Date().toISOString();
    if (!sectionStartTimes[quizStepIndex]) {
      setSectionStartTimes((prev) => ({ ...prev, [quizStepIndex]: sectionStartTime }));
    }

    const updateTimer = () => {
      const start = new Date(sectionStartTime).getTime();
      const elapsedSeconds = Math.floor((Date.now() - start) / 1000);
      const remaining = Math.max(0, timerLimitMinutes * 60 - elapsedSeconds);
  ...
  }, [quizStepIndex, tutorialDismissed, currentAssessment, sectionStartTimes]);
  ...
  if (currentStep === 1) {
    const handleVerify = () => {
      if (!token.trim()) {
        setVerificationError("Token harus diisi");
        return;
      }
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2 md:col-span-2">
                  ...
                  <Input
                    id="nama"
                    placeholder="Nama akan terisi otomatis setelah verifikasi token"
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="rank"
                    placeholder="Rank akan terisi otomatis setelah verifikasi token"
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="vessel"
                    placeholder="Nama akademi akan terisi otomatis setelah verifikasi token"
