...
// BatchReportSnapshot is a full denormalized copy of a Report's state at the time
// a batch is closed. One row per (batch, report) pair.
type BatchReportSnapshot struct {
	ID         int       `json:"id" gorm:"column:id;primaryKey;autoIncrement"`
	BatchID    int       `json:"batchId" gorm:"column:batch_id;not null;index:idx_snap_batch"`
	ReportID   int       `json:"reportId" gorm:"column:report_id;not null;index:idx_snap_report"`
	SnapshotAt time.Time `json:"snapshotAt" gorm:"column:snapshotted_at;not null"`
	...
	CaseHistory   string `json:"caseHistory" gorm:"column:case_history;type:text"`
	YearOfCase    string `json:"yearOfCase" gorm:"column:year_of_case"`

	// Data History Vessel
	VesselHistory string `json:"vesselHistory" gorm:"column:vessel_history;type:text"`
	...
	Strength              string `json:"strength" gorm:"column:strength;type:text"`
...
func (BatchReportSnapshot) TableName() string {
	return "batch_report_snapshots"
}
